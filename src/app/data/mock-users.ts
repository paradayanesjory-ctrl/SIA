import { UserCredentials } from '../models/student.model';

export const MOCK_USERS: UserCredentials[] = [
  { codigo: '192099', documento: '1091355245', password: 'Jordy123#' },
  { codigo: '192260', documento: '5073908', password: '12345678' },
  { codigo: '191919', documento: '1092355246', password: '12345678' },
  { codigo: '242113', documento: '1065872030', password: 'Valeria2000' },
  { codigo: '952902', documento: '1014991705', password: 'Realmadrid_15' }
];
